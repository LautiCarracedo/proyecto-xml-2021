...
class Ventana(Frame):
    ...
    def __init__(self, master):
        ...
        self.label_nroboleta_o_codbarra1 = Label(self.frame, textvariable=self.text_barra_or_boleta, pady=10,padx=20)
        self.label_nroboleta_o_codbarra1.grid(row=5,column=0,sticky='N' )
        ...
        self.input_nroboleta_o_codbarra1 = Text(self.frame, height = 15, width = 23)
        ...
        self.label_importe_o_codbarra2 = Label(self.frame,textvariable=self.text_importe_or_barra,pady=10,padx=20)
        self.label_importe_o_codbarra2.grid(row=5,column=2,sticky='N')

        self.input_importe_o_codbarra2 = Text(self.frame, height = 15, width = 15)
        self.input_importe_o_codbarra2.grid(row=5, column=3)
        ...
        self.input_fechapago = Text(self.frame, height = 15, width = 10)
        self.input_fechapago.grid(row=5, column=5, sticky='N')

        #Cant cuotas
        self.label_cant_cuotas = Label(self.frame,text='Cant cuotas:',pady=10,padx=20 )
        self.label_cant_cuotas.grid(row=5,column=6,sticky='N')

        self.input_cant_cuotas = Text(self.frame, height = 15, width = 5)
        self.input_cant_cuotas.grid(row=5, column=7, sticky='N')

        #Cuota Actual
        self.text_cuotaactual = StringVar()
        self.text_cuotaactual.set("Cuota Actual:")
        self.label_cuotaactual = Label(self.frame,textvariable=self.text_cuotaactual,pady=10,padx=20, state="disabled")
                ...
                if validacion_dato_origen:
                    if validacion_dato_banco:
                        if validacion_datos_cant_cuotas:
                                    ...
                                    if validacion_dato_fec_rendicion:
                                        if validacion_dato_codbarra1 and validacion_dato_codbarra2:
                                            if validacion_vector_comisiones_p_calculo:
                                                ...
                                                generador = Generador(dato_origen, dato_banco, dato_fec_rendicion, None, None, datos_fec_pagos, datos_cant_cuotas, None, dato_codbarra1, dato_codbarra2)
                                                generador.generar_xml(dato_origen, dato_banco, dato_fec_rendicion, None, None, datos_fec_pagos, datos_cant_cuotas, None, dato_codbarra1, dato_codbarra2)    
                            ...
                            messagebox.showerror(message="Para Cordobesa debe ingresar 12 o 18. Para pagos presenciales (00082-PagoFacil y 00079-Rapipago) ingresar P. Para el resto ingresar C o D según corresponda.", title="Error en las cantidad cuotas")
                    else:
                                        ...
                                        if validacion_dato_fec_rendicion:
                                            if validacion_datos_cant_vectores:
                                                if validacion_vector_comisiones_p_calculo:
                                                    ...
                                                    generador = Generador(dato_origen, dato_banco, dato_fec_rendicion, datos_boletas, datos_importes, datos_fec_pagos, datos_cant_cuotas, datos_cuota_actual, None, None)
                                                    generador.generar_xml(dato_origen, dato_banco, dato_fec_rendicion, datos_boletas, datos_importes, datos_fec_pagos, datos_cant_cuotas, datos_cuota_actual, None, None)
                                                    ...
                                                    messagebox.showerror(message="Revisar los valores ingresados en el campo cantidad de cuotas. Para Cordobesa(00935) ingresar 12 o 18. Para pagos presenciales (00082-PagoFacil y 00079-Rapipago) ingresar P. Para el resto ingresar C o D según corresponda.", title="Error en cantidad cuotas")                                                                                                                                                                                                                                                                            
                                            else:
    ...
    def mostrar_nombre_y_datosacargar_banco(self, event):
        ...
        if banco_selec == "00935":
            ...
            self.input_importe_o_codbarra2 = Text(self.frame, height = 15, width = 23)
            self.input_importe_o_codbarra2.grid(row=5, column=3)
            ...
            self.input_importe_o_codbarra2 = Text(self.frame, height = 15, width = 43)
            self.input_importe_o_codbarra2.grid(row=5, column=3)
    ...
    def verificar_cant_registros(self):
        try:
            ...
            cant_boletas = Label(self.frame, text='Cant registros: ' + str(len(vector_boletas_o_codbarra1)), pady=10,padx=20 )
            cant_boletas.grid(row=6, column=1)
            
            cant_importes = Label(self.frame, text='Cant registros: ' + str(len(vector_importes_o_codbarra2)), pady=10,padx=20 )
